  ...
  from: vi.fn((table: string) => {
    ...
    if (table === "service_areas") {
      const builder: any = {
        ...
        single: vi.fn(async () => ({
          data: { facility_id: "fac1" },
          error: null,
        })),
        delete: vi.fn(() => ({
          eq: vi.fn((_col: string, _val: any) => ({ error: null })),
        })),
        insert: vi.fn(async (_rows: any[]) => ({
          data: [{ id: "new1" }],
          error: null,
        })),
        update: vi.fn((_updates: any) => ({
    ...
    if (table === "users") {
      const builder: any = {
        ...
        eq: vi.fn((_col: string, _val: any) => builder),
        single: vi.fn(async () => ({
          data: { id: "user1", role: "admin", org_id: "org1" },
          error: null,
        })),
      };
      return builder;
...
vi.mock("../lib/rbac", () => {
  return {
    canViewServiceAreas: vi.fn(async () => ({
      id: "user1",
      role: "admin",
      org_id: "org1",
    })),
    canManageServiceAreas: vi.fn(async () => ({
      id: "user1",
      role: "admin",
      org_id: "org1",
    })),
...
describe("Service Areas API", () => {
  ...
  it("GET returns areas for a facility (happy path)", async () => {
    ...
    const req = makeRequest(
      "http://localhost/api/service-areas?facilityId=fac1"
    );
  ...
  it("POST returns 403 when not allowed", async () => {
    ...
    const req = makeRequest("http://localhost/api/service-areas", {
      facilityId: "fac1",
      areas: [{ lat: 1, lng: 2, radiusMiles: 10 }],
    });
